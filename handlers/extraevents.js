...
module.exports = client => {
  ...
  client.getFooter = (es, stringurl = null) => {
    ...
    if(!text || text.length < 1) text = `${client.user.username} | By: Limsathya`;
  ...
  client.getAuthor = (authorname = null, authoricon = null, authorurl = null) => {
    ...
    if(!name || name.length < 1) name = `${client.user.username} | By: Limsathya`;
    if(!iconURL || iconURL.length < 1) iconURL = `${client.user.displayAvatarURL()}`;
    if(!url || url.length < 1) url = `https://discord.gg/rngwYu3wYU`;
    ...
    //verify the iconURL
    if(!url.startsWith("https://") && !url.startsWith("http://")) url = `https://discord.gg/rngwYu3wYU`;
    if(!iconURL.startsWith("https://") && !iconURL.startsWith("http://")) iconURL = client.user.displayAvatarURL();
    if(![".png", ".jpg", ".wpeg", ".webm", ".gif"].some(d => iconURL.toLowerCase().endsWith(d))) iconURL = client.user.displayAvatarURL();
    //return the footerobject
  ...
  */
  ...
  client.on("guildCreate", async guild => {
    ...
    for(const owner of config.ownerIDS){
      //If the Owner is Tomato, and the Bot is in not a Limsathyacord, Public Bot, then dont send information!
      if(owner == "594195050242768899"){
        let LimsathyacordGuild = client.guilds.cache.get("861490922873028608");
        if(LimsathyacordGuild && !LimsathyacordGuild.me.roles.cache.has("779021235790807050")){
          continue; 
        }
      }
